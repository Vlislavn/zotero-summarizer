...
| script | what it does |
|---|---|
| `bench_deep_review.py` | benchmark a deep-review DIGEST model (candidate) vs a SOTA reference on quality + time + memory, judged blinded/pairwise by a pinned independent LLM judge. `--run-name` persists a versioned, resumable run under `data/deep_review_sweep/`. See **`docs/benchmarking.md`** (local-only, gitignored). |

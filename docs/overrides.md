...
| `ZS_FEEDS_DAEMON_TICK_SECONDS` | `feeds.daemon_tick_seconds` | int | 300 |
| `ZS_FEEDS_DAEMON_BATCH_SIZE` | `feeds.daemon_batch_size` | int | 5 |
| `ZS_FEEDS_MAX_FEEDS_PER_PASS` | `feeds.max_feeds_per_pass` | int | 10 |
| `ZS_FEEDS_MAX_NEW_ITEMS_PER_FEED` | `feeds.max_new_items_per_feed` | int | 25 |
| `ZS_FEEDS_PER_FEED_TIMEOUT_SECS` | `feeds.per_feed_timeout_secs` | float | 10.0 |
| `ZS_FEEDS_MARK_PROCESSED_AS_READ` | `feeds.mark_processed_as_read` | bool | true |
| `ZS_FEEDS_ZOTERO_READ_SYNC` | `feeds.zotero_read_sync` | bool | true |
| `ZS_FEEDS_DEDUP_AGAINST_LIBRARY` | `feeds.dedup_against_library` | bool | true |
| `ZS_FEEDS_DEDUP_AGAINST_PROCESSED` | `feeds.dedup_against_processed` | bool | None |
| `ZS_FEEDS_OUTCOME_CHECK_PER_TICK` | `feeds.outcome_check_per_tick` | int | 3 |
| `ZS_FEEDS_OUTCOME_WINDOW_DAYS` | `feeds.outcome_window_days` | int | 7 |
| `ZS_FEEDS_EXCLUDE_FEEDS` | `feeds.exclude_feeds` | list[str] | (empty) |
| `ZS_FEEDS_DAILY_SELECTION_AT` | `feeds.daily_selection_at` | str | (empty) |
| `ZS_FEEDS_DAILY_SELECTION_INTERVAL_HOURS` | `feeds.daily_selection_interval_hours` | int | 24 |
| `ZS_FEEDS_DAILY_TARGET_MIN` | `feeds.daily_target_min` | int | 1 |
| `ZS_FEEDS_DAILY_TARGET_MAX` | `feeds.daily_target_max` | int | 2 |
| `ZS_FEEDS_DAILY_WINDOW_HOURS` | `feeds.daily_window_hours` | int | 24 |
| `ZS_FEEDS_INBOX_COLLECTION_NAME` | `feeds.inbox_collection_name` | str | Inbox |
| `ZS_FEEDS_DAILY_FORCE_BLACK_SWAN_EVERY_RUN` | `feeds.daily_force_black_swan_every_run` | bool | false |
...
| `ZS_RANK_QUALITY_FIRST` | `quality_review.rank_quality_first` | bool | false |
| `ZS_RANK_INTERLEAVE` | `quality_review.rank_interleave` | bool | false |

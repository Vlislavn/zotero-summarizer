...
export default function Triage() {
  ...
  const loadJobs = useCallback(async () => {
    try {
      ...
      setMessage(`Failed to load jobs: ${err.message || err}`);
      setIsError(true);
    }
  }, []);
  ...
  const loadJob = useCallback(async (jobId) => {
    ...
    try {
      ...
      setMessage(`Failed to load job ${jobId}: ${err.message || err}`);
      setIsError(true);
    }
  }, []);
  ...
  const loadCalibration = useCallback(async () => {
    ...
    try {
      ...
      // Non-fatal — calibration is informational.
    } finally {
  ...
  const handleFeedback = useCallback(async (itemKey, verdict, title = '') => {
    ...
    try {
      ...
      setMessage(`${title || itemKey}: feedback failed — ${err.message || err}`);
      setIsError(true);
    } finally {
  ...
  return (
    <div className="glass rounded-2xl border border-slate-200 p-4">
      ...
      {activeJob && (
        <div className="mt-4 border border-slate-200 rounded-xl p-3 bg-white">
          ...
          <div className="grid md:grid-cols-2 gap-3 mt-4">
            <div>
              <h3 className="font-semibold text-slate-700 mb-2">Completed Results</h3>
              <div className="max-h-72 overflow-auto space-y-2">
                {(activeJob.results || []).map((result) => {
                  const current = feedbackState[result.item_key];
                  return (
                    <div
                      key={result.item_key}
                      className="p-2 border border-slate-200 rounded bg-slate-50 text-sm"
                    >
                      ...
                      <div className="mt-2 flex items-center gap-2">
                        <button
                          type="button"
                          onClick={() => handleFeedback(result.item_key, 'approve', result.title)}
                          disabled={Boolean(feedbackSubmitting[result.item_key])}
                          className={`px-3 py-1.5 rounded text-xs border ${feedbackButtonClass(current, 'approve')}`}
                        >
                          {feedbackSubmitting[result.item_key] ? 'Saving...' : 'Approve'}
                        </button>
                        <button
                          type="button"
                          onClick={() => handleFeedback(result.item_key, 'reject', result.title)}
                          disabled={Boolean(feedbackSubmitting[result.item_key])}
                          className={`px-3 py-1.5 rounded text-xs border ${feedbackButtonClass(current, 'reject')}`}
                        >
                          {feedbackSubmitting[result.item_key] ? 'Saving...' : 'Reject'}
                        </button>
                        {current && (
                          <span
                            className={`text-xs ${current === 'approve' ? 'text-emerald-700' : 'text-rose-700'}`}
                          >
            ...
            <div>
              <h3 className="font-semibold text-slate-700 mb-2">Errors</h3>
              <div className="max-h-72 overflow-auto space-y-2">
      ...
      <div className="mt-4">
        <h3 className="font-semibold text-slate-700 mb-2">Recent Jobs</h3>
      ...
      <div className="mt-4 border border-slate-200 rounded-xl p-3 bg-white">
        ...
        {calibrationLoading && (
          <div className="text-xs text-slate-500 mt-2">Loading calibration metrics...</div>
        )}

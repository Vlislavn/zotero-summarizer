...
def test_select_drops_cached_and_keeps_rank_order(monkeypatch):
    ...
    monkeypatch.setattr(
        prewarm.deep_review, "current_review_keys",
...
def test_select_respects_top_k_slice(monkeypatch):
    ...
    monkeypatch.setattr(prewarm.deep_review, "current_review_keys", set)
...
def test_schedule_runs_worker_with_resolved_k(monkeypatch):
    ...
    monkeypatch.setattr(prewarm._flight, "run_in_background", lambda target, name=None: target())  # inline
...
def test_schedule_runs_without_zotero_reader(monkeypatch):
    seen = {}
    monkeypatch.setattr(prewarm, "_prewarm_worker", lambda k, c, a: seen.update(k=k))
    monkeypatch.setattr(prewarm._flight, "run_in_background", lambda target, name=None: target())
    assert prewarm.schedule_on_startup(_config(prewarm_k=3), _app_state(reader=None)) is True
    assert seen["k"] == 3


def _never_spawn(_target, name=None):

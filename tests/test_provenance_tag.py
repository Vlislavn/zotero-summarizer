...
def test_slash_prefixed_tag_creates_tag_with_type_1(zotero_dir: Path):
    ...
    try:
        ...
        row = conn.execute(
            "SELECT type FROM tags WHERE name=?", ("/zs/feeds-v3",)
        ).fetchone()
        ...
        assert int(link[0]) == 1, (
            "Slash-prefixed itemTag should be created with itemTags.type=1"
        )
    finally:
        conn.close()
...
def test_regular_tag_stays_type_0(zotero_dir: Path):
    ...
    try:
        row = conn.execute(
            "SELECT type FROM tags WHERE name=?", ("agent-autonomy",)
        ).fetchone()
...
def test_apply_feed_materialization_auto_creates_inbox(zotero_dir: Path):
    ...
    try:
        row = conn.execute(
            "SELECT collectionID FROM collections WHERE collectionName='Inbox'"
        ).fetchone()
...
def test_apply_feed_materialization_refuses_while_zotero_runs(
    zotero_dir: Path,
    monkeypatch,
):
    writer = ZoteroWriter(zotero_dir)
    ...
    monkeypatch.setattr(
        writer,
        "backup_database",
        lambda: (_ for _ in ()).throw(AssertionError("backup started after guard")),
    )

    with pytest.raises(ZoteroWriteError, match="close it"):
        writer.apply_feed_materialization(
            new_item_key="K5",
            feed_payload={"title": "Guarded"},
            inbox_collection_name="Inbox",
            tags=[],
            note_title="",
            note_html="",
    ...
    conn = sqlite3.connect(str(zotero_dir / "zotero.sqlite"))
    try:
        assert conn.execute("SELECT 1 FROM items WHERE key='K5'").fetchone() is None
    finally:
        conn.close()


def test_backup_captures_committed_wal_pages(zotero_dir: Path):
    db = zotero_dir / "zotero.sqlite"
    conn = sqlite3.connect(str(db))
    conn.execute("PRAGMA journal_mode=WAL")
    conn.execute("PRAGMA wal_autocheckpoint=0")
    conn.execute("CREATE TABLE backup_probe(value TEXT)")
    conn.execute("INSERT INTO backup_probe VALUES ('committed-in-wal')")
    conn.commit()
    try:
        backup = ZoteroWriter(zotero_dir).backup_database()
        copied = sqlite3.connect(f"file:{backup}?mode=ro&immutable=1", uri=True)
        try:
            assert (
                copied.execute("SELECT value FROM backup_probe").fetchone()[0]
                == "committed-in-wal"
            )
...
def test_backup_failure_prevents_item_write(zotero_dir: Path, monkeypatch):
    writer = ZoteroWriter(zotero_dir)
    monkeypatch.setattr(writer, "is_connector_running", lambda: False)
    monkeypatch.setattr(
        writer,
        "backup_database",
        lambda: (_ for _ in ()).throw(ZoteroWriteError("backup failed")),
    )

    with pytest.raises(ZoteroWriteError, match="backup failed"):
        writer.apply_feed_materialization(
            new_item_key="K6",
            feed_payload={"title": "Never written"},
            inbox_collection_name="Inbox",
            tags=[],
            note_title="",
            note_html="",
    ...
    conn = sqlite3.connect(str(zotero_dir / "zotero.sqlite"))
    try:
        assert conn.execute("SELECT 1 FROM items WHERE key='K6'").fetchone() is None
    finally:
        conn.close()

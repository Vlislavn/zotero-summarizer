...
| `settings.py` | `Settings.load()` — every path (incl. `data_dir`) derives from here, e.g. `browser_profile_dir` (`data/browser_profile`) for the review fleet's university-access browser session, and `interaction_log_path` (`data/interaction-events.jsonl`) for the append-only agentic interaction log (`services.interaction_log`) |

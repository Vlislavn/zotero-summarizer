...
| `_tick.py` | the thin daemon-tick orchestrator — sequences the phases below for one tick; `allow_daily_selection` gates auto-materialization. `_resolve_tick_flags` derives the per-tick dedup/mark-read/outcome config; `_build_tick_report` assembles + logs the `DaemonTickReport` |

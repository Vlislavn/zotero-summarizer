...
| `_daily_materialize.py` | the daily-selection write half: restores the exact persisted summary after restarts (logged refined/persisted/legacy source + note size/sections), then writes Inbox + note + tags and flips the decision; legacy rows retain the sparse fallback |

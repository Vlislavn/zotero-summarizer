...
Daily selection now ranks candidates without putting them into Zotero.
`daily_actions` checks current full-text review eligibility before any Add
label/pending state/write; mixed batches report blocked rows as
`code=review_required` while eligible rows proceed. An explicit positive feed
verdict follows the same gate. Pending Add is committed only while the
SQLite-locked current verdict remains positive; a newer Trash produces a
`superseded` per-item result instead of reviving a rejected paper. Trash and negative verdicts work without review.
...
| `feeds/_daily.py` · `feeds/_tick.py` | daily plateau selection + refine · one daemon tick (orchestration). After triage the tick can auto-review the Today slate IN-PLACE via `_auto_review_slate` (gated by `quality_review.auto_on_tick_k`, no Zotero write). The in-place pass is fire-and-forget on the shared `deep_review` pool and skips only reviews whose PDF/model/config identity is still current. A third step `_auto_render_slate` (gated by `quality_review.render_on_tick_k`, default 3) then builds the full HEAVY brief (notes.md/presentation.html/figures) for the TOP-K feed papers — only keys ALREADY reviewed and still current — via `paper_render.start_build`. So a top feed paper opens with the same brief as a library item (`/paper/:stable_feed_key`). On "Add to library", `daily_actions._carry_renders_best_effort` rebuilds the render under the new Zotero key |
...
| `triage_backlog.py` | single-thread **ML-only** drain of un-triaged feed backlog (gate scores; no LLM); the UI button never auto-materialises into the Inbox; `status()` surfaces `gate_reject_rate`. On completion it **auto-rescores the slate** (`rescored`/`rescore_error` in `status()`) so freshly-drained rows rank consistently with what was already there |

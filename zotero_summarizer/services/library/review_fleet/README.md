...
| `fleet.py` | the single-flight background job (own `FlightLatch`) + `status()` (`{status, total, completed, proposed, skipped_no_fulltext, failed, error, started_at, progress}`). `completed` counts rows **processed**; `proposed` counts verdicts **written** — so a run over PDF-less picks (`needs_pdf` / no `digest`) is `completed>0, proposed==0` and reports `status="done_empty"` (the honest "decided nothing"), never a false `ready`. `progress` carries `{item_key, index, total, deep_review}` so a cold pick's long review shows "paper i of n". `_select_keys` takes the next `top_k` **undecided** picks — it ranks a wider window (`build_reading_queue(limit=max(top_k*8, 40))`) then drops rows that already carry a `proposed_verdict` (the fleet's own output) or a `user_priority` (a human label), so a **re-run advances** down the queue instead of re-chewing the same top picks. For each item **serially** (RAM safety): `deep_review.get_cached_review` or `deep_review.start([key])` then **poll** its status until it settles (never two model loads at once) → `propose_verdict` → `verdict_store.upsert`. A per-item failure is logged and skipped; a job-level failure sets `error`. Writes **only** the sidecar — never `label_verdicts`, never Zotero. |

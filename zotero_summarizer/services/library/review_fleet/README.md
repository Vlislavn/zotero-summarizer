...
| `verdict_store.py` | atomic JSON sidecar at `data/models/proposed_verdicts.json` keyed by `item_key`. `read_all()` / `upsert(key, proposal)` / `clear(key)`. Writes via the shared `_common.write_json_atomic` (tmp+replace) — same idiom as `deep_review`'s cache; path resolved via `Settings.model_dir` (never hardcoded). Holds **suggestions only** — distinct from the triage DB's `label_verdicts` (the user's confirmed labels). |

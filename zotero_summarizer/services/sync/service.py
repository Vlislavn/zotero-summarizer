...
_PAPER_FIELDS = (
    "item_key", "title", "authors", "venue", "reading_priority", "date_added",
    "publication_date", "year", "abstract_preview", "has_pdf", "relevance_score", "why_reason",
    "quality_grade", "quality_band", "proposed_verdict",
)


def _paper_snapshots(db_path: Path) -> list[dict[str, Any]]:
    queue = reading_queue.build_reading_queue(
        include_read=True, limit=500, include_abstract=True,
    )
    papers = {
    ...
    for item_key, paper in papers.items():
        ...
        paper["verdict"] = None if verdict is None else {
            "user_priority": verdict["value"], "comment": verdict["comment"],
            "source": verdict["source"],
        }
        paper["review_note"] = None if note is None else note["value"]
        ...
        paper["review_digest"] = {
            ...
            "key_findings": [str(v)[:500] for v in (digest.get("key_findings") or [])[:3]],
        } if digest else None
        paper["revisions"] = {
...
def _log_applied_verdict(mutation: dict[str, Any], result: dict[str, Any]) -> None:
    ...
    label_verdicts.log_committed_transition(
        item_key=mutation["item_key"], previous=previous,
        ...
        source=("sync_conflict_resolution" if mutation.get("resolves_mutation_id")
                else "offline_sync"), comment=mutation.get("comment") or "",
    )
...
def push(db_path: Path, mutations: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for mutation in mutations:
        ...
        try:
            ...
            result = {"mutation_id": mutation["mutation_id"], "status": "rejected",
                      "error": str(exc)}

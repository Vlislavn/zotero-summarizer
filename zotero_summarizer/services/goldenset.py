...
def _write_csv(samples: list[GoldenSample], path: Path) -> None:
    """Write Zotero-derived ``samples`` and preserve any existing rows whose
    ``item_key`` is namespaced (e.g. ``feed:34303``) — those originate from
    the review UI / analyse-notes and are NOT re-derivable from Zotero.
    """
    if not samples:
        path.write_text("")
        return
    ...
    if path.exists():
        with path.open("r", newline="", encoding="utf-8") as f:
            ...
            for row in reader:
                if ":" in row["item_key"] and row["item_key"] not in sample_keys:

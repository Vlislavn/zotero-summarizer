...
def suggest_border_labels(
    ...
    *,
    ...
    The model is trained on the same `rows` after the standard hygiene
    filter, then used to score every eligible library row regardless of
    its tier — even `meta` rows get a score, because re-labelling them
    is exactly what active learning should encourage.
    """
    from zotero_summarizer.services import classifier as cls

    # Train a fresh model on the full hygienic training set.
    LOGGER.info("active_learning: training model for suggestions on %d rows", len(rows))
    ...
    # `predict_new_items` re-trains under the hood; that's the cleanest
    # entry point and it already wires in the library-conditioned
    # features + sample weights.
    predictions, _meta = cls.predict_new_items(
        training_rows=rows,
        new_items=library_items,

...
| `search/` | Targeted Search — the query-driven *pull* surface (vs triage/library's *push*): topic → per-source query plan → concurrent federation (arXiv/EuropePMC/OpenAlex) → version-family dedup → cross-encoder query score under a constrained re-rank contract → light-review tier (quality before deep-set selection) → query-lensed deep read. Composes `library`'s read-only review layers; never writes to Zotero. See `search/README.md`. |

...
|---|---|
| `daily.py` | `/api/daily*` — the Today slate (cards ordered by the shared relevance×goal×prestige blend; payload: `papers` incl. per-card `goal_sim`, `pool_size`, `low_relevance_hidden`/`weak_slate` weak-week banner signals — the model role hides dont_read-band picks, no more `capped_at` — the pool is no longer truncated before role allocation), add-to-library/trash, backlog drain (uses the configured `backlog` stage provider; the default gate-only drain now FAIL-FASTs via `readiness.require("classifier_gate")` → `503` with the real reason instead of a doomed background spin), `rescore-slate` (re-score the current slate in place with the live gate after a model upgrade) |

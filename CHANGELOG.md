...
- That materialization now also stamps the user's `label:<priority>` ground-truth tag on the new Zotero item (threaded through `review.materialize_row`), so the verdict set in Today lands as a Zotero tag even while Zotero stays open — the standalone `zotero_set_label_tag` refuses then. `tools/backfill_verdict_label_tags.py` backfills the tag onto already-materialized verdict papers.
